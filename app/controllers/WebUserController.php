...
class WebUserController extends \BaseController {
    ...
    public function userTripStatus() {
        ...
        if ($request != NULL) {
            ...
            elseif ($request->status == 1 && $request->confirmed_walker == 0) {
                $status = 7;
            }
            elseif ($request->is_timedout == 1 && $request->current_walker != 0) {
                $status = 8;
            } 
            echo $status;
        }
    ...
    public function saveUserRequestTrip() {
        ...
        if(Input::has('my_address')){
            $my_address = Input::get('my_address');	
        }
        else{
            $my_address = "Address not available";
        }
        ...
        if ($type == "") {
            ...
            if (empty($typewalkers)) {
                ...
                $request->src_address = $my_address;
    ...
    public function userForgotPassword() {
        ...
        if ($owner) {
            ...
            $pattern = array('name' => $owner->first_name . " " . $owner->last_name, 'contact_us_email' => $admin_email, 'new_password' => $new_password, 'login_url' => $login_url);
            $subject = "Your New Password";
    ...
    public function userTrips() {
        ...
        $requests = Requests::where('owner_id', $owner_id)
                ...
                ->leftJoin('walker', 'walker.id', '=', 'request.confirmed_walker',function($join){
                    $join->where('walker.deleted_at','=',Null)
                          ->where('walker.id','<>','');
                })
