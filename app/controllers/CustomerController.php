...
/* Cronjob counter END */
        ...
        /* AUTO OFFLINE DRIVER */
        $old_time_check = date("Y-m-d H:i:s", strtotime("-20 minutes"));
        $delete_un_rq = DB::update("UPDATE walker SET is_active = '0' WHERE updated_at <= '" . $old_time_check . "' AND is_active = '1' AND id IN (SELECT DISTINCT confirmed_walker FROM request WHERE status = '0' AND is_cancelled = '0');");
        /*  Walker::where('updated_at', '<=', $old_time_check)->where('is_active', '=', 1)->update(array('is_active' => 0)); */
        /* AUTO OFFLINE DRIVER END */
            ...
            if ($result->diff >= $timeout) {
                ...
                if (isset($request_meta->walker_id)) {
                 ...
                 //   send_notifications($owner_data_id, "owner",$title, 'No ' . $driver_keyword . ' are available right now in your area. Kindly try after sometime.');
                    ...
                    $settings = Settings::where('key', 'sms_to_admin_on_unanswered_request')->first();
                    $pattern = $settings->value;
                ...
                if ($owner_data = $this->getOwnerData($owner_id, $token, $is_admin)) {
                    ...
                    if (is_token_active($owner_data->token_expiry) || $is_admin) {
                        ...
                        foreach ($type_data as $type) {
                            $eta = $eta + (($distance - $type->base_distance) * ($type->price_per_unit_distance)) + ($time * ($type->price_per_unit_time)) + ($type->base_price);
                        }
                        $response_array = array(
                            'success' => true,
                            'time' => $time,
