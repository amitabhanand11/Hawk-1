        ...
        if (Input::has('email') && Input::has('password')) {
            ...
            if ($validator->fails()) {
                ...
                if ($walker = Walker::where('email', '=', $email)->first()) {
                    ...
                    $response_array = array('success' => false, 'error' => 1, 'error_messages' => array(1), 'error_code' => 404);
                    $response_code = 200;
                }
            }
			...
			if($walker->login_by == $login_by){
				...
				if ($walker = Walker::where('social_unique_id', '=', $social_unique_id)->first()) {
                    ...
                    $response_array = array('success' => false, 'error' => 11, 'error_messages' => array(11), 'error_code' => 407);
                    $response_code = 200;
              }
			  
			 } else {
                    $response_array = array('success' => false, 'error' => 1, 'error_messages' => array(1), 'error_code' => 407);
                    $response_code = 200;
              }  
                        ...
                        if ($request->is_cancelled != 1) {
                            if ($request->current_walker == $walker_id) {
                                if ($accepted == 1) {
                                    ...
                                    // Send SMS 
                                    $owner = Owner::find($request->owner_id);
                                    $settings = Settings::where('key', 'sms_on_acceptance_of_request_by_SP')->first();
                                    ...
                                    $settings = Settings::where('key', 'sms_to_admin_on_completion_of_request')->first();
                                    $pattern = $settings->value;
                                    $pattern = str_replace('%user%', $owner->first_name . " " . $owner->last_name, $pattern);
                        ...
                        if ($request->confirmed_walker == $walker_id) {
                            ...
                            if ($request->is_completed == 1) {
                                ...
                                if ($pt_new->base_price != 0) {
                                    ...
                                    $bill['price_per_unit_distance'] = ($pt_new->price_per_unit_distance);
                                    $bill['price_per_unit_time'] = ($pt_new->price_per_unit_time);
                                } else {

                                    $bill['price_per_unit_distance'] = ($ps_new->price_per_unit_distance);

                                    $bill['price_per_unit_time'] = ($ps_new->price_per_unit_time);
                                }
                                ...
                                $tot = ($request->total - $request->ledger_payment - $request->promo_payment);
                                if ($tot <= 0) {
                                    $tot = 0;
                                }
                                $bill['total'] = $tot;
                        ...
                        if ($request = Requests::find($request_id)) {
                            if ($request->confirmed_walker == $walker_id) {
                                ...
                                if ($request->confirmed_walker != 0) {
                                    ...
                                    $user_type = 0;
                                    $id = $request->owner_id;
                                    $title = transl('walker_started_push', $id, $user_type);
                                    send_notifications($request->owner_id, "owner", $title, $message);
                        ...
                        if ($request = Requests::find($request_id)) {
                            if ($request->confirmed_walker == $walker_id) {
                                ...
                                if ($request->is_walker_started == 1) {
                                    ...
                                    $id = $request->owner_id;
                                    $user_type = 0;
                                    $title = transl('walker_arrived_push', $id, $user_type);
                                    ...
                                    // Send SMS 
                                    $owner = Owner::find($request->owner_id);
                                    $settings = Settings::where('key', 'sms_on_arrival_of_SP')->first();
                                    $pattern = $settings->value;
                                    $pattern = str_replace('%user%', $owner->first_name . " " . $owner->last_name, $pattern);
                                    $pattern = str_replace('%driver%', $walker->first_name . " " . $walker->last_name, $pattern);
                                    $pattern = str_replace('%driver_mobile%', $walker->phone, $pattern);
                        ...
                        if ($request = Requests::find($request_id)) {
                            if ($request->confirmed_walker == $walker_id) {
                                ...
                                if ($request->is_walker_arrived == 1) {
                                    ...
                                    $user_type = 0;
                                    $id = $request->owner_id;
                                    $title = transl('walk_started_push', $id, $user_type);
                        ...
                        if ($request = Requests::find($request_id)) {
                            ...
                            if ($request->confirmed_walker == $walker_id) {
                                ...
                                if ($request->is_started == 1) {
                                    ...
                                    $id = $request->owner_id;
                                    $user_type = 0;
                                    $title = transl('walk_end_push', $id, $user_type);
                                    ...
                                    // Send SMS 
                                    $owner = Owner::find($request->owner_id);
                                    $settings = Settings::where('key', 'sms_on_completion_of_by_SP')->first();
                                    $pattern = $settings->value;
                                    $pattern = str_replace('%user%', $owner->first_name . " " . $owner->last_name, $pattern);
                                    $pattern = str_replace('%driver%', $walker->first_name . " " . $walker->last_name, $pattern);
                                    $pattern = str_replace('%driver_mobile%', $walker->phone, $pattern);
                                    ...
                                    $subject = "Invoice Generated";
                                    email_notification($request->owner_id, 'owner', $detail_owner, $subject, 'invoice');
                    ...
                    foreach ($request_data as $data) {
                        ...
                        if ($is_multiple_service->value == 0) {
                            ...
                            if ($pt_new->base_price != 0) {
                                ...
                                $request['price_per_unit_distance'] = ($pt_new->price_per_unit_distance);
                                $request['price_per_unit_time'] = ($pt_new->price_per_unit_time);
                            } else {

                                $request['price_per_unit_distance'] = ($ps_new->price_per_unit_distance);

                                $request['price_per_unit_time'] = ($ps_new->price_per_unit_time);
                            }
                        ...
                        $request['distance_cost'] = ($data->distance_cost);
                        $request['time_cost'] = ($data->time_cost);
                        $tot = ($data->total - $data->ledger_payment - $data->promo_payment);
                        if ($tot <= 0) {
                            $tot = 0;
                        }
                        $request['total'] = $tot;
