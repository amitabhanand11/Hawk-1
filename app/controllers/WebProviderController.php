...
class WebProviderController extends \BaseController {
    ...
    public function providerRequestPing() {
        ...
        $query = "SELECT id,src_address,latitude,longitude,owner_id,TIMESTAMPDIFF(SECOND,request_start_time, '$time') as diff from request where is_cancelled = 0 and status = 0 and current_walker=$walker_id and TIMESTAMPDIFF(SECOND,request_start_time, '$time') <= $provider_timeout limit 1";
        $requests = DB::select(DB::raw($query));
        $request_data = array();
        foreach ($requests as $request) {
            $request_data['success'] = "true";
            $request_data['request_id'] = $request->id;
            $request_data['time_left_to_respond'] =  $provider_timeout - $request->diff;
            ...
            $request_data['owner']['my_address'] = $request->src_address;
    ...
    public function providerForgotPassword() {
        ...
        if ($walker) {
            ...
            $pattern = array('name' => ucwords($walker->first_name . " " . $walker->last_name), 'contact_us_email' => $admin_email, 'new_password' => $new_password, 'login_url' => $login_url);
            $subject = "Your New Password";
    ...
    public function decline_request() {
        ...
        if ($request->current_walker == $walker_id) {
            ...
            if (isset($request_meta->walker_id)) {
                ...
                // request ended
                Requests::where('id', '=', $request_id)->update(array('current_walker' => 0, 'status' => 1,'is_cancelled' => 1));
            }
        }
        return Redirect::to('/provider/trips');
    }
